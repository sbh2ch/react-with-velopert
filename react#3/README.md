...
이러한 번거러움을 redux를 사용하여 줄일 수 있다.

redux를 사용하면 구조가 바뀐다.

### store
프로젝트의 상태를 모두 담고 있는 `store`가 있다.
컴포넌트가 필요한 정보를 `props`로 전달한다.
만약 어플리케이션에서 어떤 변화가 일어나면, (예를들어 input의 텍스트 정보가 바뀌거나 모달이 열렸다 닫혔다.) `action`을 `reducer`에게 전달한다.

### reducer
`reducer`은 변화를 일으켜주는 함수이다.
순수해야 한다. 특정 값에 대한 결과는 항상 같아야한다.
Math.random(), date 등 사용금지
값을 내부에서 직접 수정하면 안된다.
`reducer`는 여러개가 될 수 있다. (inputReducer, modalReducer 등등)

### action
`action`은 종류(type)와 정보(payload)가 담긴 객체이다. 
`action`을 `reducer`에 넣어주면 다음 `state`를 리턴한다.


## Redux 업데이트 절차
![process](./assets/process.png)
예를들어 모달을 닫는 액션이 들어왔다고 가정하면
1. 컴포넌트가 액션을 리듀서에게 디스패치한다 
2. 루트리듀서가 서브리듀서에게 너네중에 모달 처리할수 있는 리듀서가 있는지 확인한다.
3. 서브리듀서 중 처리할 수 있는 리듀서가 다음 스테이트를 리턴한다.
...
여기서 중요한 부분은 이전까지는 컴포넌트 내에서 상태변화가 일어났었는데 이제 컴포넌트 바깥(스토어, 리듀서)으로 이동했다.

### Redux를 꼭 사용해야 할까? 컴포넌트의 state는 사용하지 말아야 하나?
만약 redux를 사용하지 않고 충분히 개발가능하면 사용하지 않아도 된다.
ui에 종속적인 정보라면 굳이 redux store에 관리 할 필요가 없음. 내부에서만 사용되고 자주 사용된다면 그냥 state에서 관리하는게 더 나음.
